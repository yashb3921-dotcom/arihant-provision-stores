    ...
    <div className="max-w-3xl mx-auto py-12 animate-fade-in-up">
      ...
      <div className="glass p-8 sm:p-10 rounded-[3rem] border border-white shadow-[0_20px_60px_rgba(0,0,0,0.05)] mb-12 flex flex-col sm:flex-row items-center gap-8 relative overflow-hidden">
          ...
          <div className="text-center sm:text-left flex-1 relative z-10">
              ...
              <div className="inline-flex items-center gap-2 bg-white/80 text-slate-700 px-5 py-2.5 rounded-2xl text-sm font-black uppercase tracking-widest border border-slate-200 shadow-sm">
                  <Package size={16}/> {myOrders.length} Lifetime Orders
              </div>
          </div>
          
          <button onClick={logout} className="sm:ml-auto w-full sm:w-auto px-8 py-4 bg-red-50 text-red-600 rounded-2xl font-bold text-lg flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all shadow-sm active:scale-95 z-10 border border-red-100">
      ...
      {myOrders.map(order => (
          <div key={order.id} className="bg-white p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-xl transition-all group">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 mb-6">
                ...
                <div className="text-left sm:text-right bg-slate-50 p-4 rounded-2xl border border-slate-100 w-full sm:w-auto shadow-inner">
                    ...
                    <p className="text-base font-black text-slate-800">#{order.id.slice(0,8).toUpperCase()}</p>
                </div>
            </div>
            ...
            <div className="pt-5 border-t-2 border-slate-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-slate-600 bg-white border border-slate-200 px-4 py-2 rounded-2xl shadow-sm">
