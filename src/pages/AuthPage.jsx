...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(err.message === 'Invalid login credentials' ? 'Incorrect Number/Email or Password.' : err.message);
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="glass p-8 sm:p-12 rounded-[3rem] border border-white/80 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {form.isSignup && (
              <div className="space-y-2">
                ...
                <div className="relative group">
                    <User className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={24} />
                    <input type="text" placeholder="e.g. Rahul Sharma" required className="w-full py-5 pl-16 bg-white/90 border-2 border-slate-200 rounded-2xl text-lg font-bold outline-none focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-sm" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                </div>
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <Smartphone className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={24} />
                <input type="text" placeholder="e.g. 9876543210" required className="w-full py-5 pl-16 bg-white/90 border-2 border-slate-200 rounded-2xl text-lg font-bold outline-none focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-sm" value={form.identifier} onChange={e => setForm({...form, identifier: e.target.value})} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <KeyRound className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors" size={24} />
                <input type={showPassword ? "text" : "password"} placeholder="••••••••" required className="w-full py-5 pl-16 pr-14 bg-white/90 border-2 border-slate-200 rounded-2xl text-lg font-bold outline-none focus:bg-white focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-sm" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
            ...
            <button disabled={loading} className="w-full min-h-[72px] bg-gradient-to-r from-orange-500 to-amber-500 text-white py-5 rounded-full font-extrabold text-2xl hover:from-orange-600 hover:to-amber-600 shadow-xl shadow-orange-500/40 hover:shadow-orange-500/60 transition-all duration-300 hover:-translate-y-1 active:scale-[0.98] mt-10 flex justify-center items-center border border-orange-400">
                {loading ? <Loader className="animate-spin" size={32} /> : (form.isSignup ? 'Create Account' : 'Secure Login')}
            </button>
          </form>
          
          <div className="mt-10 pt-8 border-t-2 border-slate-200/60 text-center">
            <button onClick={() => { setForm({ identifier: '', password: '', name: '', isSignup: !form.isSignup }); setShowPassword(false); }} className="text-lg font-bold text-slate-600 hover:text-orange-600 transition-colors p-2">
