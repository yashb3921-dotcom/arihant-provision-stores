      ...
      <div className="w-full max-w-md animate-fade-in-up">
        <div className="glass p-10 sm:p-12 rounded-[3rem] border border-white/80 shadow-[0_20px_60px_rgba(0,0,0,0.08)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {form.isSignup && (
              <div className="space-y-2">
                <label className="text-xs font-black text-slate-500 ml-2 uppercase tracking-widest">Full Name</label>
                <div className="relative group">
                    ...
                    <input type="text" placeholder="e.g. Rahul Sharma" required className="w-full p-4.5 pl-14 bg-white/90 border-2 border-slate-200 rounded-2xl font-bold outline-none focus:bg-white focus:border-orange-400 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-inner" value={form.name} onChange={e => setForm({...form, name: e.target.value})} />
                </div>
              </div>
            )}
            
            <div className="space-y-2">
              <label className="text-xs font-black text-slate-500 ml-2 uppercase tracking-widest">Mobile Number or Admin Email</label>
              <div className="relative group">
                ...
                <input type="text" placeholder="e.g. 9876543210" required className="w-full p-4.5 pl-14 bg-white/90 border-2 border-slate-200 rounded-2xl font-bold outline-none focus:bg-white focus:border-orange-400 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-inner" value={form.identifier} onChange={e => setForm({...form, identifier: e.target.value})} />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                ...
                <input type={showPassword ? "text" : "password"} placeholder="••••••••" required className="w-full p-4.5 pl-14 pr-12 bg-white/90 border-2 border-slate-200 rounded-2xl font-bold outline-none focus:bg-white focus:border-orange-400 focus:ring-4 focus:ring-orange-500/20 transition-all placeholder:text-slate-400 shadow-inner" value={form.password} onChange={e => setForm({...form, password: e.target.value})} />
          ...
          <div className="mt-10 pt-8 border-t-2 border-slate-200/60 text-center">
            <button onClick={() => { setForm({ identifier: '', password: '', name: '', isSignup: !form.isSignup }); setShowPassword(false); }} className="text-base font-bold text-slate-600 hover:text-orange-600 transition-colors">
