      ...
      <div className="absolute inset-y-0 right-0 w-full max-w-md bg-white/95 backdrop-blur-2xl shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 ease-out border-l border-white">
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-white/50">
          <h2 className="text-2xl font-black flex items-center gap-3 tracking-tight text-slate-900">
            <div className="p-2 bg-gradient-to-tr from-orange-100 to-amber-50 rounded-xl text-orange-600 shadow-inner"><ShoppingBag size={24} /></div>
            Your Cart
          </h2>
          <button onClick={() => setIsCartOpen(false)} className="p-2 bg-slate-50 text-slate-500 rounded-full hover:bg-red-50 hover:text-red-500 transition-all hover:rotate-90 duration-300 border border-slate-100"><X size={20}/></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
          {cart.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-slate-400 opacity-80 animate-in fade-in zoom-in duration-500">
              ...
              <p className="font-bold text-xl text-slate-500 text-center">Your cart is empty.</p>
              <p className="text-sm font-medium mt-2 text-center">Looks like you haven't added anything yet.</p>
            </div>
          ) : (
            cart.map((item, index) => {
              ...
              return (
                <div key={item.id} className="flex gap-4 p-4 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 group" style={{ animationDelay: `${index * 50}ms` }}>
                  <div className="w-24 h-24 rounded-2xl overflow-hidden bg-slate-50 relative shrink-0">
                    {hasDisc && <div className="absolute top-0 left-0 bg-gradient-to-r from-red-500 to-orange-500 text-white text-[8px] font-black px-1.5 py-0.5 rounded-br-lg z-10 shadow-sm">{item.discount_percent}% OFF</div>}
                  ...
                  <div className="flex-1 flex flex-col py-1">
                    ...
                    <div className="flex items-center justify-between mt-auto pt-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">{item.unit}</p>
        ...
        {cart.length > 0 && (
          <div className="p-6 bg-white border-t border-slate-100 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] z-10">
            <div className="flex justify-between items-end mb-6">
              <span className="text-slate-500 font-bold uppercase tracking-widest text-xs mb-1">Total Amount</span>
              <span className="text-4xl font-black text-slate-900 tracking-tight">₹{cartTotal}</span>
            </div>
            <button onClick={handleCheckout} disabled={loading} className="w-full bg-gradient-to-r from-orange-500 to-amber-500 text-white py-4.5 rounded-full font-bold text-lg hover:from-orange-600 hover:to-amber-600 transition-all duration-300 flex items-center justify-center gap-2 shadow-lg shadow-orange-500/30 hover:shadow-orange-500/50 hover:-translate-y-0.5 disabled:opacity-70 group active:scale-[0.98]">
